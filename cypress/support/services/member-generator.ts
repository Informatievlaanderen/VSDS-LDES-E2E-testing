    ...
    waitAvailable() {
        return cy.exec(`docker ps -f "name=${this.serviceName}$" -q`)
            .then(result => {
                ...
                return cy.waitUntil(() => this.isReady(containerId), { timeout: timeouts.ready, interval: timeouts.fastCheck, errorMsg: `Timed out waiting for container '${this.serviceName}' to be available` });
            });
    }
