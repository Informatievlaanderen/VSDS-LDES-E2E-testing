...
export class TestMessageSink {
    ...
    checkCount(collectionName: string, count: number, checkFn: (actual: number, expected: number) => boolean = (x, y) => x === y) {
        return cy.waitUntil(() => this.hasCount(collectionName, count, checkFn), { timeout: timeouts.ready, interval: timeouts.check, errorMsg: `Timed out waiting for sink collection '${collectionName}' to correctly compare to ${count}` });
    }
