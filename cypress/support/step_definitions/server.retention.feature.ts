...
import { createAndStartService, stopAndRemoveService, currentIngestedMemberCount, server, testPartialPath, testDatabase } from "./common_step_definitions";
...
When('eventually there are at least {int} members in the database', (expected: number) => {
    cy.waitUntil(() => currentIngestedMemberCount().then(count => count >= expected), { timeout: timeouts.ready, interval: timeouts.slowCheck, errorMsg: `Timed out waiting for database '${testDatabase()}' ingest count to be at least ${expected}` });
})

When('eventually there are {int} members in the database', (expected: number) => {
    cy.waitUntil(() => currentIngestedMemberCount().then(count => count === expected), { timeout: timeouts.ready, interval: timeouts.slowCheck, errorMsg: `Timed out waiting for database '${testDatabase()}' ingest count to be ${expected}` });
})
...
Then('the member count remains around {int} for {int} seconds', (expected: number, seconds: number) => {
    ...
    cy.waitUntil(() => currentIngestedMemberCount().then(count => cy.log(`Current member count: ${count}`)
        .then(() => {
...
Then('eventually the member count remains constant for {int} seconds', (seconds: number) => {
    ...
    currentIngestedMemberCount().then(count => cy.log(`Last member count: ${count}`).then(() => lastCount = count))
        .then(() => {
            const timer = new PerformanceTimer();
            cy.waitUntil(
                () => currentIngestedMemberCount().then(count => cy.log(`Current member count: ${count}`).then(() => {
