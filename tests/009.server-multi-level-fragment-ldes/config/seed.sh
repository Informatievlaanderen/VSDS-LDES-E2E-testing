...
curl --fail -X POST 'http://localhost:8080/admin/api/v1/eventstreams/mobility-hindrances/views' -H 'Content-Type: text/turtle' -d "@$SCRIPT_PATH/mobility-hindrances.by-location-and-time.ttl"
code=$?
if [ $code != 0 ] 
    then exit $code
fi
